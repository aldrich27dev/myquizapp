...
if (handleNextQuestion < allQuestions.length) {
  ...
  return (
    <div className='flex w-full h-screen justify-center items-center bg-gradient-to-r from-yellow-300 to to-white-400'>
          ...
          <div className='w-full'>
              {
                allQuestions[currentQuestion].options.map((options) => (
                  <button className='m-2 h-10 w-24 rounded-md bg-slate 300 hover:bg-slate-400 hover:shadow-xl transition-all duration-500 ease-in-out' onClick={()=>handleAnswerOptions(options.isCorrect)}>{options.answerText}</button>
                ))
              }
